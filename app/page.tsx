...
import MermaidVisualizer from '@/components/MermaidVisualizer'; // (Phase 4에서 추가할 예정)

export default function Home() {
  const [activeTab, setActiveTab] = useState<'code' | 'mermaid'>('code');
      ...
      <header className="h-14 bg-white border-b border-neutral-200 flex items-center px-6 gap-4 shrink-0 shadow-sm z-10">
        ...
        <button
          ...
          className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${
            activeTab === 'code' ? 'bg-blue-100 text-blue-700' : 'hover:bg-neutral-100 text-neutral-600'
          }`}
        >
          💻 코드 스니펫 모드
        </button>
        <button
          ...
          className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${
            activeTab === 'mermaid' ? 'bg-blue-100 text-blue-700' : 'hover:bg-neutral-100 text-neutral-600'
          }`}
        >
          📊 다이어그램 모드
        </button>
